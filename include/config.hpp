...
namespace ConfigHelper {
	...
	struct Config {
		...
		std::string getValAsString(const std::string& key, std::string defaultValue="") const {
			ConfigMap::const_iterator itr = cm.find(key);
		...
		int get(const std::string& key, const int N, A& dst) const {
			...
			int i=0;
			for(int cnt=0; i<N && cnt<line.length(); ++i) {
				int skip=0;
				T val;
				sscanf(start+cnt,"%*[^0-9-+.eE]%lf%n",&val,&skip);
				if(skip<=0) {
					throw std::runtime_error("[Config::get error] not enough data!");
				}
				dst[i]=val;
				cnt+=skip;
			}
		...
		inline bool load(std::string fn) {
			...
			if (ps!=PS_DONE) {
				throw std::runtime_error("[Config::load error] parsing config file failed!");
			}
			std::cout<<std::endl;
			return true;
		}
	...
	protected:
		...
		{
			if (state==PS_BEGIN) {
				if(!parseLine(line,key,val)) return PS_FAIL;
				ArrayState as=arrayState(val);
				switch(as) {
		case AS_BEGIN: return PS_ARRAY;
		...
		case AS_END: return PS_DONE;
				...
				trim(nline);
				if(nline.empty()) {
					std::cout<<"[Config::parseLine warn] skip empty line in array!"<<std::endl;
					return PS_ARRAY;
				}
				std::string oline=cm[key];
				if(oline[oline.length()-1]==';' || oline[oline.length()-1]=='[')
					cm[key].append(nline);
				else
