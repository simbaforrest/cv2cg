...
inline double computeRMSError(const Mat &error)
{
...
inline double computeNCC(const Mat& w, const Mat& t)
{
	Scalar mw, mt, dw, dt;
	meanStdDev(w, mw, dw);
	meanStdDev(t, mt, dt);
...
void HomoESM::track(Mat &testImage, int nIters, Mat &H,
                    double &rmsError, double&ncc,
                    Ptr<LieAlgebra> lieAlgebra,
                    bool saveComputations,
                    vector<HomoESMState> *computations) const
{
